...
var config = {
  'consumerKey': 'ca6ab97e77970d9e50ce52e7f9cddc8b',
  'consumerSecret': '98f816efb63c7008',
  ...
  if (!config.accessToken || !config.accessSecret) {
    ...
    getAccessTokenSecretPair(api, function (accessToken, accessTokenSecret) {
      ...
      getUserData(api, function (error, data) {
        debug(data, 'response');
        console.log('Hello 1: ' + data.auth_user.first_name);
        var serverModel = require('upwork-api/serverModel');
        //    serverModel.queryDatabases(function(res,callback) {
        //      console.log("inner");
        //      console.log(res);
        //       var Search = require('upwork-api/lib/routers/jobs/search.js').Search;
        //   var freelancers = new Search(api);
        //   var params = {'q': 'dotnet', 'title': res};
        //   freelancers.find(params, function(error, data) {
        //     console.log(data);
        //   });
        //   });
        serverModel.queryDatabases(function (error, data) {
          console.log("inner"); console.log(res);
        });


        //   var server = require('upwork-api/server.js').server;
        //   var serverdata = new server(api);
        //  var params = {'q': 'dotnet', 'title': 'dotnet Developer'};
        //   serverdata.find(params, function(error, data) {
        //     console.log(error)
        //   console.log(data);
        // });
    ...
    api.setAccessToken(config.accessToken, config.accessSecret, function () {
      ...
      getUserData(api, function (error, data) {
        debug(data, 'response');
        // server_time
        console.log('Hello 2 : ' + data.auth_user.first_name);
        var Search = require('upwork-api/lib/routers/jobs/search.js').Search;

        var freelancers = new Search(api);
        var params = { 'q': 'dotnet', 'title': 'Web Developer' };
        freelancers.find(params, function (error, data) {
          console.log(data);
        });
      });
